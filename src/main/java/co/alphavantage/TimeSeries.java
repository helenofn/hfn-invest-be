...
import co.alphavantage.request.timeseries.Interval;
import co.alphavantage.request.timeseries.OutputSize;
import co.alphavantage.response.timeseries.Daily;
import co.alphavantage.response.timeseries.DailyAdjusted;
import co.alphavantage.response.timeseries.IntraDay;
import co.alphavantage.response.timeseries.Monthly;
import co.alphavantage.response.timeseries.MonthlyAdjusted;
import co.alphavantage.response.timeseries.Weekly;
import co.alphavantage.response.timeseries.WeeklyAdjusted;

/**
 * The Stock Time Series Data provides realtime and historical equity data in 4
 * different temporal resolutions: (1) intraday, (2) daily, (3) weekly, and (4)
 * monthly.
 */
public class TimeSeries {
	 ...
	 * volume) of the equity specified, updated realtime.
	 *
	 * @param symbol     the stock symbol to lookup
	 * @param interval   the interval between two consecutive data points in the
	 *                   time series {@link Interval}
	 * @param outputSize the specification of the amount of returned data points
	 *                   {@link OutputSize}
	 ...
	 *
	 * @param symbol   the stock symbol to lookup
	 * @param interval the interval between two consecutive data points in the time
	 *                 series {@link Interval}
	 * @return {@link IntraDay} time series data
	 */
	 ...
	 * daily close, daily volume) of the equity specified.
	 *
	 * @param symbol     the stock symbol to lookup
	 * @param outputSize the specification of the amount of returned data points
	 *                   {@link OutputSize}
	...
	/**
	 * This API returns weekly time series (last trading day of each week, weekly
	 * open, weekly high, weekly low, weekly close, weekly volume).
	 *
	 * @param symbol the stock symbol to lookup
	...
	/**
	 * This API returns weekly adjusted time series (last trading day of each week,
	...
	/**
	 * This API returns monthly time series (last trading day of each month, monthly
	 ...
	 * adjusted close, monthly volume, monthly dividend)
	 *
	 * @param symbol the stock symbol to lookup
